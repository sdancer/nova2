defmodule Nova.Compiler.TypeChecker do
  ...
  def infer_many(env, exprs) do
      ...
      go = Nova.Runtime.fix4(fn go -> fn auto_arg0 -> fn auto_arg1 -> fn auto_arg2 -> fn auto_arg3 -> case {auto_arg0, auto_arg1, auto_arg2, auto_arg3} do
...
case Nova.Compiler.Unify.unify(ty, Nova.Compiler.Types.t_array(elem_ty)) do
  ...
  def infer_clauses(env, scrut_ty, result_ty, clauses, init_sub) do
      ...
      expr_to_pattern = Nova.Runtime.fix(fn expr_to_pattern -> fn auto_arg0 -> case auto_arg0 do
      ...
      infer_guard_expr = Nova.Runtime.fix2(fn infer_guard_expr -> fn auto_arg0 -> fn auto_arg1 -> case {auto_arg0, auto_arg1} do
  ...
  {:right, res} -> {:right, %{env: res.env, sub: res.sub}}
      ...
      infer_guard = fn auto_arg0 -> fn auto_arg1 -> case {auto_arg0, auto_arg1} do
        {e, :nothing} -> {:right, %{env: e, sub: Nova.Compiler.Types.empty_subst()}}
        {e, ({:just, guard_expr})} -> infer_guard_expr.(e).(guard_expr)
      end end end
      go = Nova.Runtime.fix5(fn go -> fn auto_arg0 -> fn auto_arg1 -> fn auto_arg2 -> fn auto_arg3 -> fn auto_arg4 -> case {auto_arg0, auto_arg1, auto_arg2, auto_arg3, auto_arg4} do
  ...
  def type_expr_to_type_with_all_aliases(alias_map, param_alias_map, var_map, ({:ty_expr_tuple, ts})) do
    Nova.Compiler.Types.t_tuple(Nova.Runtime.map(fn auto_p0 -> type_expr_to_type_with_all_aliases(alias_map, param_alias_map, var_map, auto_p0) end, ts))
  end
  ...
  def type_expr_to_type_with_env(env, alias_map, param_alias_map, var_map, ({:ty_expr_tuple, ts})) do
    Nova.Compiler.Types.t_tuple(Nova.Runtime.map(fn auto_p0 -> type_expr_to_type_with_env(env, alias_map, param_alias_map, var_map, auto_p0) end, ts))
  end
  ...
  def substitute_type_expr(alias_map, param_alias_map, param_subst, ({:ty_expr_tuple, ts})) do
    Nova.Compiler.Types.t_tuple(Nova.Runtime.map(fn auto_p0 -> substitute_type_expr(alias_map, param_alias_map, param_subst, auto_p0) end, ts))
  end
  ...
  def type_expr_to_type(var_map, ({:ty_expr_tuple, ts})) do
    Nova.Compiler.Types.t_tuple(Nova.Runtime.map(fn auto_p0 -> type_expr_to_type(var_map, auto_p0) end, ts))
  end
  ...
  def process_import_decl(registry, env, imp) do
    case Nova.Compiler.Types.lookup_module(registry, imp.module_name) do
      ...
      {:just, exports} -> if imp.hiding do
          ...
          if Nova.Array.null(imp.items) do
            Nova.Compiler.Types.merge_exports_to_env(env, exports)
          else
            Nova.Array.foldl(fn auto_p0 -> fn auto_p1 -> import_item(exports, auto_p0, auto_p1) end end, env, imp.items)
          end
        end
    end
  end
    ...
    case item do
      ...
      {:import_type, type_name, spec} -> case Nova.Map.lookup(type_name, exports.types) do
          ...
          {:just, type_info} -> case spec do
              ...
              {:import_some, ctor_names} -> Nova.Compiler.Types.merge_type_export(env, exports, type_name, ctor_names)
              :import_none -> env
            end
        end
    end
  ...
  type_info = %{arity: Nova.Array.length(dt.type_vars), constructors: Nova.Runtime.map(& &1.name, dt.constructors)}
  exp1 = %{exp | types: Nova.Map.insert(dt.name, type_info, exp.types)}
  Nova.Array.foldl(add_constructor_placeholder.(dt), exp1, dt.constructors)
        {exp, ({:decl_type_alias, ta})} -> %{exp | type_aliases: Nova.Map.insert(ta.name, %{params: ta.type_vars, body: ta.ty}, exp.type_aliases)}
