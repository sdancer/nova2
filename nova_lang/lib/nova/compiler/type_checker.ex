defmodule Nova.Compiler.TypeChecker do
  ...
  def instantiate(env, scheme) do
      ...
      go = Nova.Runtime.fix3(fn go -> fn __arg0__ -> fn __arg1__ -> fn __arg2__ -> case {__arg0__, __arg1__, __arg2__} do
        {e, [], sub} -> %{ty: Nova.Compiler.Types.apply_subst(sub, scheme.ty), env: e}
        {e, vars, sub} -> case Nova.Array.uncons(vars) do
  :nothing -> %{ty: Nova.Compiler.Types.apply_subst(sub, scheme.ty), env: e}
  {:just, %{head: v, tail: rest}} -> 
      ...
      go.(e_prime).(rest).(sub_prime)
end
      end end end end end)
      go.(env).(scheme.vars).(Nova.Map.empty)
  end
  ...
  def infer(env, ({:expr_qualified, m, name})) do
      ...
      full_name = Nova.Runtime.append(m, Nova.Runtime.append(".", name))
  ...
  {:left, e} -> {:left, e}
  {:right, pat_res} -> 
      inner_expr = if Nova.Array.null(rest_pats) do
        body
      else
  ...
  def infer(env, ({:expr_record_update, rec, updates})) do
    infer_record_update(env, rec, updates)
  end
  ...
  def infer_record_update(env, rec, updates) do
    ...
    {:left, err} -> {:left, err}
    {:right, rec_res} ->
      case infer_fields(rec_res.env, updates) do
        {:left, err} -> {:left, err}
        {:right, update_res} ->
...
infer_do(let_res.env, rest)
  ...
  def infer_many(env, exprs) do
      ...
      go = Nova.Runtime.fix4(fn go -> fn __arg0__ -> fn __arg1__ -> fn __arg2__ -> fn __arg3__ -> case {__arg0__, __arg1__, __arg2__, __arg3__} do
        {e, [], acc, sub} -> {:right, %{tys: Nova.Array.reverse(acc), sub: sub, env: e}}
        {e, es, acc, sub} -> case Nova.Array.uncons(es) do
  :nothing -> {:right, %{tys: Nova.Array.reverse(acc), sub: sub, env: e}}
  {:just, %{head: expr, tail: rest}} -> case infer(e, expr) do
      {:left, err} -> {:left, err}
      {:right, res} -> go.(res.env).(rest).([res.ty | acc]).(Nova.Compiler.Types.compose_subst(res.sub, sub))
    end
end
      end end end end end end)
      go.(env).(exprs).([]).(Nova.Compiler.Types.empty_subst())
  end
  ...
  def infer_elems(env, elem_ty, elems) do
      ...
      go = Nova.Runtime.fix4(fn go -> fn __arg0__ -> fn __arg1__ -> fn __arg2__ -> fn __arg3__ -> case {__arg0__, __arg1__, __arg2__, __arg3__} do
        {e, _, [], sub} -> {:right, %{sub: sub, env: e}}
        {e, e_ty, es, sub} -> case Nova.Array.uncons(es) do
  :nothing -> {:right, %{sub: sub, env: e}}
  {:just, %{head: expr, tail: rest}} -> case infer(e, expr) do
      {:left, err} -> {:left, err}
      {:right, res} -> case Nova.Compiler.Unify.unify(Nova.Compiler.Types.apply_subst(res.sub, e_ty), res.ty) do
          {:left, ue} -> {:left, {:unify_err, ue}}
          {:right, s2} -> go.(res.env).(Nova.Compiler.Types.apply_subst(s2, e_ty)).(rest).(Nova.Compiler.Types.compose_subst(s2, Nova.Compiler.Types.compose_subst(res.sub, sub)))
        end
    end
end
      end end end end end end)
      go.(env).(elem_ty).(elems).(Nova.Compiler.Types.empty_subst())
  end

  # @type fields_result :: %{tys: array()(tuple()(string())(type())), sub: subst(), env: env()}

  def infer_fields(env, fields) do
    
      go = Nova.Runtime.fix4(fn go -> fn __arg0__ -> fn __arg1__ -> fn __arg2__ -> fn __arg3__ -> case {__arg0__, __arg1__, __arg2__, __arg3__} do
        {e, [], acc, sub} -> {:right, %{tys: Nova.Array.reverse(acc), sub: sub, env: e}}
        {e, fs, acc, sub} -> case Nova.Array.uncons(fs) do
  :nothing -> {:right, %{tys: Nova.Array.reverse(acc), sub: sub, env: e}}
  {:just, %{head: {:tuple, name, expr}, tail: rest}} -> case infer(e, expr) do
      {:left, err} -> {:left, err}
      {:right, res} -> go.(res.env).(rest).([{:tuple, name, res.ty} | acc]).(Nova.Compiler.Types.compose_subst(res.sub, sub))
    end
end
      end end end end end end)
      go.(env).(fields).([]).(Nova.Compiler.Types.empty_subst())
  end
  ...
  def infer_pat(env, ({:pat_con, con_name, pats}), ty) do
    case Nova.Compiler.Types.lookup_env(env, con_name) do
...
{:right, %{env: env_prime, sub: pat_res.sub}}
  ...
  def infer_record_pat(env, fields, ty) do
      ...
      go = Nova.Runtime.fix4(fn go -> fn __arg0__ -> fn __arg1__ -> fn __arg2__ -> fn __arg3__ -> case {__arg0__, __arg1__, __arg2__, __arg3__} do
        {e, [], field_types, sub} -> {:tuple, row_var, e_prime} = Nova.Compiler.Types.fresh_var(e, "row")
...
case Nova.Compiler.Unify.unify(Nova.Compiler.Types.apply_subst(sub, ty), expected_rec) do
  ...
  {:right, s} -> {:right, %{env: e_prime, sub: Nova.Compiler.Types.compose_subst(s, sub)}}
        ...
        {e, fs, field_types, sub} -> case Nova.Array.uncons(fs) do
  :nothing -> go.(e).([]).(field_types).(sub)
  {:just, %{head: {:tuple, label, pat}, tail: rest}} ->     {:tuple, field_var, e1} = Nova.Compiler.Types.fresh_var(e, Nova.Runtime.append("f_", label))
    field_ty = {:ty_var, field_var}
  case infer_pat(e1, pat, field_ty) do
    {:left, err} -> {:left, err}
    {:right, pat_res} ->
go.(pat_res.env).(rest).(Nova.Map.insert(label, Nova.Compiler.Types.apply_subst(pat_res.sub, field_ty), field_types)).(Nova.Compiler.Types.compose_subst(pat_res.sub, sub))
  end
end
      end end end end end end)
      go.(env).(fields).(Nova.Map.empty).(Nova.Compiler.Types.empty_subst())
  end

  def infer_list_pat(env, pats, ty) do
    
      go_elems = Nova.Runtime.fix4(fn go_elems -> fn __arg0__ -> fn __arg1__ -> fn __arg2__ -> fn __arg3__ -> case {__arg0__, __arg1__, __arg2__, __arg3__} do
        {e, [], _, sub} -> {:right, %{env: e, sub: sub}}
        {e, ps, e_ty, sub} -> case Nova.Array.uncons(ps) do
  :nothing -> {:right, %{env: e, sub: sub}}
  {:just, %{head: p, tail: rest}} ->   case infer_pat(e, p, e_ty) do
    {:left, err} -> {:left, err}
    {:right, pat_res} ->
...
{:right, %{env: tl_res.env, sub: Nova.Compiler.Types.compose_subst(tl_res.sub, Nova.Compiler.Types.compose_subst(hd_res.sub, s1))}}
  ...
  def infer_con_pats(env, con_ty, pats, result_ty) do
      ...
      go = Nova.Runtime.fix4(fn go -> fn __arg0__ -> fn __arg1__ -> fn __arg2__ -> fn __arg3__ -> case {__arg0__, __arg1__, __arg2__, __arg3__} do
        {e, ty, [], sub} -> case Nova.Compiler.Unify.unify(ty, result_ty) do
  {:left, ue} -> {:left, {:unify_err, ue}}
  {:right, s} -> {:right, %{env: e, sub: Nova.Compiler.Types.compose_subst(s, sub)}}
end
        {e, ty, ps, sub} -> case Nova.Array.uncons(ps) do
  :nothing -> go.(e).(ty).([]).(sub)
  {:just, %{head: p, tail: rest}} -> case ty do
      {:ty_con, c} ->
        cond do
          ((c.name == "Fun") and (Nova.Array.length(c.args) == 2)) -> case %{a: Nova.Array.head(c.args), b: Nova.Array.last(c.args)} do
              %{a: {:just, arg_ty}, b: {:just, res_ty}} -> case infer_pat(e, p, arg_ty) do
                  {:left, err} -> {:left, err}
                  {:right, pat_res} -> go.(pat_res.env).(res_ty).(rest).(Nova.Compiler.Types.compose_subst(pat_res.sub, sub))
                end
      ...
      go.(env).(con_ty).(pats).(Nova.Compiler.Types.empty_subst())
  end
  ...
  def infer_binds(env, binds) do
      ...
      infer_binds_pass2 = Nova.Runtime.fix3(fn infer_binds_pass2 -> fn __arg0__ -> fn __arg1__ -> fn __arg2__ -> case {__arg0__, __arg1__, __arg2__} do
        {e, [], sub} -> {:right, %{env: e, sub: sub}}
        {e, bs, sub} -> case Nova.Array.uncons(bs) do
  :nothing -> {:right, %{env: e, sub: sub}}
  {:just, %{head: bind, tail: rest}} -> case infer(e, bind.value) do
      {:left, err} -> {:left, err}
      ...
      add_bind_placeholders = fn e -> fn bs -> Nova.Array.foldl(add_one, e, bs) end end
  ...
  def infer_clauses(env, scrut_ty, result_ty, clauses, init_sub) do
      ...
      expr_to_pattern = Nova.Runtime.fix(fn expr_to_pattern -> fn __arg0__ -> case __arg0__ do
        ({:expr_var, name}) -> Nova.Compiler.Ast.pat_var(name)
        ({:expr_app, ({:expr_var, con}), arg}) -> Nova.Compiler.Ast.pat_con(con, [expr_to_pattern.(arg)])
        ({:expr_app, ({:expr_app, ({:expr_var, con}), arg1}), arg2}) -> Nova.Compiler.Ast.pat_con(con, [expr_to_pattern.(arg1), expr_to_pattern.(arg2)])
        ({:expr_lit, lit}) -> Nova.Compiler.Ast.pat_lit(lit)
        ({:expr_parens, e}) -> expr_to_pattern.(e)
        _ -> :pat_wildcard
      end end end)
      infer_guard_expr = Nova.Runtime.fix2(fn infer_guard_expr -> fn __arg0__ -> fn __arg1__ -> case {__arg0__, __arg1__} do
        {e, ({:expr_bin_op, "&&", left, right})} -> case infer_guard_expr.(e).(left) do
  {:left, err} -> {:left, err}
  {:right, left_res} -> case infer_guard_expr.(left_res.env).(right) do
      ...
      {:right, right_res} -> {:right, %{env: right_res.env, sub: Nova.Compiler.Types.compose_subst(right_res.sub, left_res.sub)}}
        ...
        {e, ({:expr_bin_op, "<-", pat_expr, val_expr})} -> case infer(e, val_expr) do
  {:left, err} -> {:left, err}
  {:right, val_res} -> 
  ...
  {:right, pat_res} -> {:right, %{env: pat_res.env, sub: Nova.Compiler.Types.compose_subst(pat_res.sub, val_res.sub)}}
        ...
        {e, ({:expr_bin_op, ",", left, right})} -> infer_guard_expr.(e).(Nova.Compiler.Ast.expr_bin_op("&&", left, right))
        {e, expr} -> case infer(e, expr) do
  {:left, err} -> {:left, err}
  {:right, res} -> {:right, %{env: res.env, sub: res.sub}}
end
      end end end end)
      infer_guard = fn __arg0__ -> fn __arg1__ -> case {__arg0__, __arg1__} do
        {e, :nothing} -> {:right, %{env: e, sub: Nova.Compiler.Types.empty_subst()}}
        {e, ({:just, guard_expr})} -> infer_guard_expr.(e).(guard_expr)
      end end end
      go = Nova.Runtime.fix5(fn go -> fn __arg0__ -> fn __arg1__ -> fn __arg2__ -> fn __arg3__ -> fn __arg4__ -> case {__arg0__, __arg1__, __arg2__, __arg3__, __arg4__} do
        {e, _, _, [], sub} -> {:right, %{env: e, sub: sub}}
        {e, s_ty, r_ty, cs, sub} -> case Nova.Array.uncons(cs) do
  :nothing -> {:right, %{env: e, sub: sub}}
  ...
  {:right, guard_res} -> case infer(guard_res.env, clause.body) do
      ...
      {:right, body_res} -> case Nova.Compiler.Unify.unify(Nova.Compiler.Types.apply_subst(body_res.sub, r_ty_prime), body_res.ty) do
          ...
          {:right, s} -> 
              ...
              go.(e_prime).(s_ty).(r_ty).(rest).(new_sub)
        end
    end
end
end
      ...
      go.(env).(scrut_ty).(result_ty).(clauses).(init_sub)
  end
  ...
  def check_function(env, func) do
      ...
      expr = if Nova.Array.null(func.parameters) do
        func.body
      else
        Nova.Compiler.Ast.expr_lambda(func.parameters, func.body)
      end
  ...
  def check_data_type(env, dt) do
    check_data_type_with_aliases(Nova.Map.empty, env, dt)
  end

  def check_data_type_with_aliases(alias_map, env, dt) do
    
      type_var_pairs = Nova.Array.map_with_index(fn i -> fn v -> {:tuple, v, Nova.Compiler.Types.mk_tvar((env.counter + i), v)} end end, dt.type_vars)
      new_counter = (env.counter + Nova.Array.length(dt.type_vars))
      ...
      add_constructor = fn e -> fn con -> 
        con_type = build_constructor_type_with_aliases(alias_map, type_var_map, con.fields, result_type)
      ...
      Nova.Array.foldl(add_constructor, env1, dt.constructors)
  end
  ...
  def build_constructor_type(var_map, fields, result_type) do
      ...
      go = Nova.Runtime.fix(fn go -> fn __arg0__ -> case __arg0__ do
        [] -> result_type
        fs -> case Nova.Array.uncons(fs) do
  :nothing -> result_type
  {:just, %{head: field, tail: rest}} -> 
      field_ty = type_expr_to_type(var_map, field.ty)
      Nova.Compiler.Types.t_arrow(field_ty, go.(rest))
end
      end end end)
      go.(fields)
  end
          ...
          {:just, info} ->
            cond do
              (Nova.Array.length(info.params) == Nova.Array.length(args)) -> 
                  arg_types = Nova.Runtime.map(fn __p0__ -> type_expr_to_type_with_all_aliases(alias_map, param_alias_map, var_map, __p0__) end, args)
  ...
  def type_expr_to_type_with_all_aliases(alias_map, param_alias_map, var_map, ({:ty_expr_for_all, vars, t})) do
      ...
      var_list = Nova.Array.map_with_index(fn i -> fn name -> {:tuple, name, Nova.Compiler.Types.mk_tvar(-(i + 1), name)} end end, vars)
  ...
  def type_expr_to_type_with_all_aliases(alias_map, param_alias_map, var_map, ({:ty_expr_tuple, ts})) do
    Nova.Compiler.Types.t_tuple(Nova.Runtime.map(fn __p0__ -> type_expr_to_type_with_all_aliases(alias_map, param_alias_map, var_map, __p0__) end, ts))
  end
    ...
    case collect_type_app(Nova.Compiler.Ast.ty_expr_app(f, arg)) do
      {:tuple, con_name, args} -> case Nova.Map.lookup(con_name, param_alias_map) do
          {:just, info} ->
            cond do
              (Nova.Array.length(info.params) == Nova.Array.length(args)) -> 
                  arg_types = Nova.Runtime.map(fn __p0__ -> type_expr_to_type_with_env(env, alias_map, param_alias_map, var_map, __p0__) end, args)
                  param_subst = Nova.Map.from_foldable(Nova.Array.zip(info.params, arg_types))
                  substitute_type_expr(alias_map, param_alias_map, param_subst, info.body)
  ...
  def type_expr_to_type_with_env(env, alias_map, param_alias_map, var_map, ({:ty_expr_tuple, ts})) do
    Nova.Compiler.Types.t_tuple(Nova.Runtime.map(fn __p0__ -> type_expr_to_type_with_env(env, alias_map, param_alias_map, var_map, __p0__) end, ts))
  end
    ...
    case collect_type_app(Nova.Compiler.Ast.ty_expr_app(f, arg)) do
      {:tuple, con_name, args} -> case Nova.Map.lookup(con_name, param_alias_map) do
          {:just, info} ->
            cond do
              (Nova.Array.length(info.params) == Nova.Array.length(args)) -> 
                  arg_types = Nova.Runtime.map(fn __p0__ -> substitute_type_expr(alias_map, param_alias_map, param_subst, __p0__) end, args)
  ...
  def substitute_type_expr(alias_map, param_alias_map, param_subst, ({:ty_expr_tuple, ts})) do
    Nova.Compiler.Types.t_tuple(Nova.Runtime.map(fn __p0__ -> substitute_type_expr(alias_map, param_alias_map, param_subst, __p0__) end, ts))
  end
  ...
  def type_expr_to_type(var_map, ({:ty_expr_tuple, ts})) do
    Nova.Compiler.Types.t_tuple(Nova.Runtime.map(fn __p0__ -> type_expr_to_type(var_map, __p0__) end, ts))
  end
  ...
  def collect_type_aliases(decls) do
      ...
      collect = fn m -> fn decl -> case decl do
        {:decl_type_alias, ta} -> if Nova.Array.null(ta.type_vars) do
  ...
  def collect_param_type_aliases(decls) do
      ...
      collect = fn __arg0__ -> fn __arg1__ -> case {__arg0__, __arg1__} do
        {m, ({:decl_type_alias, ta})} -> Nova.Map.insert(ta.name, %{params: ta.type_vars, body: ta.ty}, m)
        {m, _} -> m
      end end end
      Nova.Array.foldl(collect, Nova.Map.empty, decls)
  end
  ...
  def check_function_bodies(env, decls) do
      ...
      go = Nova.Runtime.fix2(fn go -> fn e -> fn ds -> case Nova.Array.uncons(ds) do
        :nothing -> {:right, e}
  ...
  def merge_multi_clause_functions(decls) do
      ...
      clause_to_case_clause = fn _ -> fn func -> case Nova.Array.head(func.parameters) do
        :nothing -> :nothing
        {:just, pat} -> {:just, %{pattern: pat, body: func.body, guard: :nothing}}
      end end end
      clause_to_tuple_case = fn _ -> fn func -> 
        n = Nova.Array.length(func.parameters)
        tup_name = if (n == 2) do
          "Tuple"
        else
  ...
  clause_to_case_clause.([]).(func)
      ...
      merge_clauses_into_one = fn clauses -> case Nova.Array.head(clauses) do
        ...
        {:just, first} -> 
            name = first.name
            num_params = Nova.Array.length(first.parameters)
            param_names = Nova.Array.map_with_index(fn i -> fn _ -> Nova.Runtime.append("__arg", Nova.Runtime.show(i)) end end, first.parameters)
            ...
            case_clauses = Nova.Array.map_maybe(clause_to_case_clause.(param_vars), clauses)
            case_expr = case num_params do
              0 -> first.body
              1 -> case Nova.Array.head(param_vars) do
                  {:just, v} -> Nova.Compiler.Ast.expr_case(v, case_clauses)
                  :nothing -> first.body
                end
              _ -> Nova.Compiler.Ast.expr_case(Nova.Compiler.Ast.expr_tuple(param_vars), Nova.Array.map_maybe(clause_to_tuple_case.(param_vars), clauses))
            end
