defmodule Nova.Compiler.CodeGenWasmSimple do
  ...
  def collect_free_vars(bound, ({:expr_let, binds, body})) do
      ...
      bind_free = Nova.Runtime.foldl((fn s -> fn b -> Nova.Set.union(s, (collect_free_vars(bound, b.value))) end end), Nova.Set.empty, binds)
  ...
  def collect_free_vars(bound, ({:expr_case, s, clauses})) do
      ...
      clause_free = Nova.Runtime.foldl((fn acc -> fn c -> 
  ...
  def collect_free_vars(bound, ({:expr_list, es})) do
    Nova.Runtime.foldl((fn s -> fn e -> Nova.Set.union(s, (collect_free_vars(bound, e))) end end), Nova.Set.empty, es)
  end
  ...
  def collect_free_vars(bound, ({:expr_record, fs})) do
    Nova.Runtime.foldl((fn s -> fn ({:tuple, _, e}) -> Nova.Set.union(s, (collect_free_vars(bound, e))) end end), Nova.Set.empty, fs)
  end
  ...
  def collect_free_vars(bound, ({:expr_record_update, e, fs})) do
    Nova.Set.union((collect_free_vars(bound, e)), (Nova.Runtime.foldl((fn s -> fn ({:tuple, _, ex}) -> Nova.Set.union(s, (collect_free_vars(bound, ex))) end end), Nova.Set.empty, fs)))
  end
  ...
  def collect_lambdas(bound, ({:expr_list, es}), state) do
    Nova.Runtime.foldl((fn s -> fn e -> collect_lambdas(bound, e, s) end end), state, es)
  end
  ...
  def collect_lambdas(bound, ({:expr_record, fs}), state) do
    Nova.Runtime.foldl((fn s -> fn ({:tuple, _, e}) -> collect_lambdas(bound, e, s) end end), state, fs)
  end
  ...
  def collect_lambdas(bound, ({:expr_record_update, e, fs}), state) do
      ...
      Nova.Runtime.foldl((fn s -> fn ({:tuple, _, ex}) -> collect_lambdas(bound, ex, s) end end), state_prime, fs)
  end
  ...
  add_ctors = fn m -> fn d -> Nova.Runtime.foldr((fn ({:tuple, i, ctor}) -> fn acc -> Nova.Map.insert(ctor.name, %{tag: i, arity: Nova.List.length(ctor.fields)}, acc) end end), m, (Nova.Array.map_with_index((fn i -> fn c -> {:tuple, i, c} end end), (Nova.Array.from_foldable(d.constructors))))) end end
