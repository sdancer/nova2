defmodule Nova.Compiler.CstParser do
  ...
  def tok_left_paren() do
    token(Cst.tok_left_paren)
  end



  def tok_right_paren() do
    token(Cst.tok_right_paren)
  end



  def tok_left_brace() do
    token(Cst.tok_left_brace)
  end



  def tok_right_brace() do
    token(Cst.tok_right_brace)
  end



  def tok_left_square() do
    token(Cst.tok_left_square)
  end



  def tok_right_square() do
    token(Cst.tok_right_square)
  end



  def tok_equals() do
    token(Cst.tok_equals)
  end
  ...
  def tok_backslash() do
    token(Cst.tok_backslash)
  end



  def tok_right_arrow() do
    token(Cst.tok_right_arrow)
  end



  def tok_left_arrow() do
    token(Cst.tok_left_arrow)
  end



  def tok_double_colon() do
    token(Cst.tok_double_colon)
  end
  ...
  def tok_right_fat_arrow() do
    token(Cst.tok_right_fat_arrow)
  end



  def tok_forall() do
    token(Cst.tok_forall)
  end
  ...
  {:just, name}
      ...
      case expect_map(extract_lower) do
  {:left, err} -> {:left, err}
  {:right, {:tuple, tok, name}} ->
    Nova.Runtime.pure(%{token: tok, name: Cst.ident(name)})
end
  end
  ...
  {:just, ({:tuple, (Nova.Runtime.map(Cst.module_name, mod_)), name})}
end
        _ -> :nothing
      end end
      case expect_map(extract_qual_lower) do
  {:left, err} -> {:left, err}
  {:right, {:tuple, tok, ({:tuple, mod_, name})}} ->
    Nova.Runtime.pure(%{token: tok, module: mod_, name: Cst.ident(name)})
end
  end
  ...
  def tok_upper_name() do
      ...
      case expect_map(extract_upper) do
  {:left, err} -> {:left, err}
  {:right, {:tuple, tok, name}} ->
    Nova.Runtime.pure(%{token: tok, name: Cst.proper(name)})
end
  end
  ...
  def tok_qualified_upper_name() do
      ...
      extract_qual_upper = fn auto_arg0 -> case auto_arg0 do
        ({:tok_upper_name, mod_, name}) -> {:just, ({:tuple, (Nova.Runtime.map(Cst.module_name, mod_)), name})}
        _ -> :nothing
      end end
      case expect_map(extract_qual_upper) do
  {:left, err} -> {:left, err}
  {:right, {:tuple, tok, ({:tuple, mod_, name})}} ->
    Nova.Runtime.pure(%{token: tok, module: mod_, name: Cst.proper(name)})
end
  end
  ...
  def tok_operator() do
      ...
      extract_op = fn auto_arg0 -> case auto_arg0 do
        ({:tok_operator, mod_, op}) -> {:just, ({:tuple, (Nova.Runtime.map(Cst.module_name, mod_)), op})}
        _ -> :nothing
      end end
      case expect_map(extract_op) do
  {:left, err} -> {:left, err}
  {:right, {:tuple, tok, ({:tuple, mod_, op})}} ->
    Nova.Runtime.pure(%{token: tok, module: mod_, name: Cst.operator(op)})
end
  end
  ...
  def layout_block(p) do
      ...
      dummy_token = %{range: %{start: %{line: 0, column: 0}, end_: %{line: 0, column: 0}}, leading_comments: [], trailing_comments: [], value: Cst.tok_layout_end(0)}
    ...
    Nova.Runtime.bind(some(parse_type_var_binding()), fn vars ->
      Nova.Runtime.bind(tok_dot(), fn dot ->
        case parse_type() do
          {:left, err} -> {:left, err}
          {:right, body} ->
            Nova.Runtime.pure((Cst.type_forall(forall_tok, vars, dot, body)))
        end
      end)
    end)
  end) end)
  ...
  def parse_type1() do
    Control.Lazy.defer(fn _ ->   case parse_type2() do
    {:left, err} -> {:left, err}
    {:right, t} ->
  ...
  case parse_type1() do
    ...
    {:right, t2} ->
      Nova.Runtime.pure(({:tuple, arr, t2}))
        ...
        case rest do
  {:just, ({:tuple, arr, t2})} -> Nova.Runtime.pure((Cst.type_constrained(t, arr, t2)))
  ...
  def parse_type2() do
    Control.Lazy.defer(fn _ ->   case parse_type3() do
    {:left, err} -> {:left, err}
    {:right, t} ->
  ...
  case parse_type2() do
    ...
    {:right, t2} ->
      Nova.Runtime.pure(({:tuple, arr, t2}))
        ...
        case rest do
  {:just, ({:tuple, arr, t2})} -> Nova.Runtime.pure((Cst.type_arrow(t, arr, t2)))
  :nothing -> Nova.Runtime.pure(t)
end
      end)
  end end)
  ...
  def parse_type3() do
    Control.Lazy.defer(fn _ ->   case parse_type_atom() do
    {:left, err} -> {:left, err}
    {:right, head} ->
  ...
  Nova.Runtime.pure((Cst.type_app(head, args)))
end
      end)
  end end)
  end
  ...
  def parse_type_var() do
    Nova.Runtime.fmap(Cst.type_var, tok_lower_name())
  end



  def parse_type_con() do
    Nova.Runtime.fmap(Cst.type_constructor, tok_qualified_upper_name())
  end
  ...
  def parse_type_parens() do
    ...
    Nova.Runtime.pure((Cst.type_parens(w)))
  end) end)
  end
  ...
  def parse_type_record() do
    ...
    Nova.Runtime.pure((Cst.type_record(w)))
  end) end)
  end
  ...
  def parse_row_label() do
    Control.Lazy.defer(fn _ ->   case parse_label() do
    {:left, err} -> {:left, err}
    {:right, label} ->
  ...
  def parse_label() do
      ...
      case expect_map(extract_label) do
  {:left, err} -> {:left, err}
  {:right, {:tuple, tok, name}} ->
    Nova.Runtime.pure(%{token: tok, name: Cst.label(name)})
end
  end
  ...
  def parse_expr1() do
    Control.Lazy.defer(fn _ ->   case parse_expr2() do
    {:left, err} -> {:left, err}
    {:right, e} ->
  ...
  case parse_type() do
    ...
    {:right, t} ->
      Nova.Runtime.pure(({:tuple, dc, t}))
        ...
        case ann do
  {:just, ({:tuple, dc, t})} -> Nova.Runtime.pure((Cst.expr_typed(e, dc, t)))
  ...
  def parse_expr2() do
    Control.Lazy.defer(fn _ ->   case parse_expr3() do
    {:left, err} -> {:left, err}
    {:right, e} ->
      parse_expr2_rest(e)
  end end)
  ...
  case parse_expr3() do
    ...
    case op_result do
  {:just, ({:tuple, op, e2})} ->   case parse_expr2_ops(([{:tuple, op, e2} | []])) do
    {:left, err} -> {:left, err}
    {:right, rest} ->
      Nova.Runtime.pure((Cst.expr_op(e, rest)))
  end
  ...
  case parse_expr5() do
    ...
    case tick_result do
  {:just, ({:tuple, wrapped, e2})} ->   case parse_expr2_infix(([{:tuple, wrapped, e2} | []])) do
    {:left, err} -> {:left, err}
    {:right, rest} ->
      Nova.Runtime.pure((Cst.expr_infix(e, rest)))
  end
  :nothing -> Nova.Runtime.pure(e)
end
  end)
  ...
  def parse_expr3() do
     ...
     case parse_expr3() do
       {:left, err} -> {:left, err}
       {:right, e} ->
         Nova.Runtime.pure((Cst.expr_negate(neg, e)))
     end
   end)
  ...
  def parse_expr4() do
    Control.Lazy.defer(fn _ ->   case parse_expr5() do
    {:left, err} -> {:left, err}
    {:right, head} ->
  ...
  Nova.Runtime.pure((Cst.expr_app(head, args)))
end
      end)
  end end)
  end
  ...
  {:just, updates} -> Nova.Runtime.pure((Cst.expr_record_update(e, updates)))
  ...
  case parse_label() do
    ...
    case access do
  {:just, %{dot: dot, path: path}} ->     access_expr = Cst.expr_record_accessor(%{expr: e, dot: dot, path: path})
  ...
  def parse_record_update_part() do
    ...
    case parse_record_update_field() do
      {:left, err} -> {:left, err}
      {:right, first} ->
    ...
    {:right, label} ->
      Nova.Runtime.bind(tok_equals(), fn eq ->
        case parse_expr() do
          {:left, err} -> {:left, err}
          {:right, value} ->
            Nova.Runtime.pure((Cst.record_update_leaf(label, eq, value)))
        end
      end)
  end
  end
            ...
            {:right, true_expr} ->
              Nova.Runtime.bind(tok_keyword("else"), fn else_kw ->
                case parse_expr() do
                  {:left, err} -> {:left, err}
                  {:right, false_expr} ->
                    Nova.Runtime.pure((Cst.expr_if(%{keyword: kw, cond_: cond_expr, then_kw: then_kw, then_branch: true_expr, else_kw: else_kw, else_branch: false_expr})))
                end
              end)
          end
        end)
    ...
    Nova.Runtime.bind(layout_block(parse_let_binding()), fn bindings ->
      Nova.Runtime.bind(tok_keyword("in"), fn in_kw ->
        case parse_expr() do
          {:left, err} -> {:left, err}
          {:right, body} ->
            Nova.Runtime.pure((Cst.expr_let(%{keyword: kw, bindings: bindings, in_: in_kw, body: body})))
        end
      end)
    end)
  end) end)
    ...
    Nova.Runtime.bind(some(parse_binder()), fn binders ->
      Nova.Runtime.bind(tok_right_arrow(), fn arr ->
        case parse_expr() do
          {:left, err} -> {:left, err}
          {:right, body} ->
            Nova.Runtime.pure((Cst.expr_lambda(%{symbol: bs, binders: binders, arrow: arr, body: body})))
        end
      end)
    end)
  end) end)
    ...
    Nova.Runtime.bind(separated(parse_expr(), tok_comma()), fn head ->
      Nova.Runtime.bind(tok_keyword("of"), fn of_kw ->
        Nova.Runtime.bind(layout_block(parse_case_branch()), fn branches ->
          Nova.Runtime.pure((Cst.expr_case(%{keyword: kw, head: head, of: of_kw, branches: branches})))
        end)
      end)
    end)
  end) end)
  ...
  Nova.Runtime.bind(layout_block(parse_let_binding()), fn bindings ->
    Nova.Runtime.pure(({:tuple, wh, bindings}))
          ...
          Nova.Runtime.pure((Cst.unconditional(arrow, %{expr: expr, bindings: where_clause})))
        end)
    end
  end) end)
  end
  ...
  def parse_guarded_exprs_arrow() do
    ...
    Nova.Runtime.pure((Cst.guarded(guards)))
  end) end)
  end
  ...
  def parse_do() do
    ...
    Nova.Runtime.bind(layout_block(parse_do_statement()), fn stmts ->
      Nova.Runtime.pure((Cst.expr_do(%{keyword: kw, statements: stmts})))
    end)
  end) end)
  end
  ...
  def parse_do_let() do
    Control.Lazy.defer(fn _ ->   Nova.Runtime.bind(tok_keyword("let"), fn kw ->
    Nova.Runtime.bind(layout_block(parse_let_binding()), fn bindings ->
      Nova.Runtime.pure((Cst.do_let(kw, bindings)))
    end)
  end) end)
  end
  ...
  def parse_do_bind() do
    Control.Lazy.defer(fn _ ->   case parse_binder() do
    {:left, err} -> {:left, err}
    {:right, binder} ->
      Nova.Runtime.bind(tok_left_arrow(), fn arr ->
        case parse_expr() do
          {:left, err} -> {:left, err}
          {:right, expr} ->
            Nova.Runtime.pure((Cst.do_bind(binder, arr, expr)))
        end
      end)
  end end)
  end
  ...
  def parse_do_discard() do
    Control.Lazy.defer(fn _ -> Nova.Runtime.fmap(Cst.do_discard, parse_expr()) end)
  end
  ...
  def parse_let_sig() do
    ...
    Nova.Runtime.bind(tok_double_colon(), fn dc ->
      case parse_type() do
        {:left, err} -> {:left, err}
        {:right, ty} ->
          Nova.Runtime.pure((Cst.let_binding_signature(%{label: name, separator: dc, value: ty})))
      end
    end)
  end) end)
  end
  ...
  Nova.Runtime.bind(layout_block(parse_let_binding()), fn bindings ->
    Nova.Runtime.pure(({:tuple, wh, bindings}))
            ...
            Nova.Runtime.pure((Cst.let_binding_pattern(pat, eq, %{expr: expr, bindings: where_clause})))
          end)
      end
    end)
  end) end)
  ...
  def parse_let_name() do
    ...
    Nova.Runtime.bind(many(parse_binder()), fn binders ->
      case parse_guarded() do
        {:left, err} -> {:left, err}
        {:right, guarded} ->
          Nova.Runtime.pure((Cst.let_binding_name(%{name: name, binders: binders, guarded: guarded})))
      end
    end)
  end) end)
  end
  ...
  Nova.Runtime.bind(layout_block(parse_let_binding()), fn bindings ->
    Nova.Runtime.pure(({:tuple, wh, bindings}))
          ...
          Nova.Runtime.pure((Cst.unconditional(eq, %{expr: expr, bindings: where_clause})))
        end)
    end
  end) end)
  end
  ...
  def parse_pattern_guard() do
    Control.Lazy.defer(fn _ ->   Nova.Runtime.bind(optional((case parse_binder() do
  {:left, err} -> {:left, err}
  {:right, b} ->
    Nova.Runtime.bind(tok_left_arrow(), fn arr ->
  ...
  def parse_expr_section() do
      Nova.Runtime.bind(token(Cst.tok_underscore), fn tok ->
    Nova.Runtime.pure((Cst.expr_section(tok)))
  end)
  end
  ...
  def parse_expr_hole() do
      Nova.Runtime.bind(tok_hole(), fn {:tuple, tok, hole_name} ->
        ...
        name = %{token: tok, name: Cst.ident(full_name)}
    Nova.Runtime.pure((Cst.expr_hole(name)))
  end)
  end
  ...
  def parse_expr_ident() do
    Nova.Runtime.fmap(Cst.expr_ident, tok_qualified_lower_name())
  end



  def parse_expr_constructor() do
    Nova.Runtime.fmap(Cst.expr_constructor, tok_qualified_upper_name())
  end
  ...
  def parse_expr_string() do
    ...
    Nova.Runtime.pure((Cst.expr_string(tok, s)))
  end)
  end
  ...
  def parse_expr_int() do
    ...
    Nova.Runtime.pure((Cst.expr_int(tok, n)))
  end)
  end
  ...
  def parse_expr_number() do
    ...
    Nova.Runtime.pure((Cst.expr_number(tok, n)))
  end)
  end
  ...
  def parse_expr_char() do
    ...
    Nova.Runtime.pure((Cst.expr_char(tok, c)))
  end)
  end
  ...
  def parse_expr_bool() do
     ...
     Nova.Runtime.pure((Cst.expr_boolean(tok, true)))
   end)
      parse_false =    Nova.Runtime.bind(tok_keyword("false"), fn tok ->
     Nova.Runtime.pure((Cst.expr_boolean(tok, false)))
   end)
  ...
  def parse_expr_array() do
    ...
    Nova.Runtime.pure((Cst.expr_array(d)))
  end) end)
  end
  ...
  def parse_expr_record() do
    ...
    Nova.Runtime.pure((Cst.expr_record(d)))
  end) end)
  end
    ...
    {:right, label} ->
      Nova.Runtime.bind(Nova.Runtime.alt(tok_colon(), tok_equals()), fn sep ->
        case parse_expr() do
          {:left, err} -> {:left, err}
          {:right, value} ->
            Nova.Runtime.pure((Cst.record_field(label, sep, value)))
        end
      end)
  end end)
  end
  ...
  def parse_record_pun() do
    Nova.Runtime.fmap(Cst.record_pun, tok_lower_name())
  end
  ...
  def parse_expr_parens() do
    ...
    Nova.Runtime.pure((Cst.expr_parens(w)))
  end) end)
  end
  ...
  def parse_binder1() do
    Control.Lazy.defer(fn _ ->   case parse_binder2() do
    {:left, err} -> {:left, err}
    {:right, b} ->
      Nova.Runtime.bind(many((Nova.Runtime.bind(tok_operator(), fn op ->
  case parse_binder2() do
    {:left, err} -> {:left, err}
    {:right, b2} ->
  ...
  Nova.Runtime.pure((Cst.binder_op(b, ops)))
end
      end)
  end end)
  end
  ...
  def parse_binder2() do
    Control.Lazy.defer(fn _ ->   case parse_binder3() do
    {:left, err} -> {:left, err}
    {:right, b} ->
      Nova.Runtime.bind(optional((Nova.Runtime.bind(tok_double_colon(), fn dc ->
  case parse_type() do
    ...
    {:right, t} ->
      Nova.Runtime.pure(({:tuple, dc, t}))
        ...
        case ann do
  {:just, ({:tuple, dc, t})} -> Nova.Runtime.pure((Cst.binder_typed(b, dc, t)))
  ...
  def parse_binder_nullary_con() do
    ...
    Nova.Runtime.pure((Cst.binder_constructor(con, [])))
  end)
  end
  ...
  def parse_binder_wildcard() do
      Nova.Runtime.bind(token(Cst.tok_underscore), fn tok ->
    Nova.Runtime.pure((Cst.binder_wildcard(tok)))
  end)
  end
  ...
  def parse_binder_hole() do
      Nova.Runtime.bind(tok_hole(), fn {:tuple, tok, hole_name} ->
        full_name = Nova.Runtime.append("_", hole_name)
        name = %{token: tok, name: Cst.ident(full_name)}
    Nova.Runtime.pure((Cst.binder_var(name)))
  end)
  end
  ...
  def parse_binder_var() do
    Control.Lazy.defer(fn _ ->   Nova.Runtime.bind(tok_lower_name(), fn name ->
    Nova.Runtime.bind(optional((Nova.Runtime.bind(token(Cst.tok_at), fn at ->
  case parse_binder_atom() do
    {:left, err} -> {:left, err}
    {:right, b} ->
      ...
      case named do
  {:just, ({:tuple, at, b})} -> Nova.Runtime.pure((Cst.binder_named(name, at, b)))
  :nothing -> Nova.Runtime.pure((Cst.binder_var(name)))
end
    end)
  end) end)
  end
  ...
  def parse_binder_string() do
      Nova.Runtime.bind(tok_string(), fn {:tuple, tok, s} ->
    Nova.Runtime.pure((Cst.binder_string(tok, s)))
  end)
  end
  ...
  Nova.Runtime.bind(tok_int(), fn {:tuple, tok, n} ->
    Nova.Runtime.pure((Cst.binder_int(neg, tok, n)))
  end)
end)
  end
  ...
  def parse_binder_number() do
      ...
      Nova.Runtime.bind(optional((satisfy(is_neg))), fn neg ->
  Nova.Runtime.bind(tok_number(), fn {:tuple, tok, n} ->
    Nova.Runtime.pure((Cst.binder_number(neg, tok, n)))
  end)
end)
  end
  ...
  def parse_binder_char() do
      Nova.Runtime.bind(tok_char(), fn {:tuple, tok, c} ->
    Nova.Runtime.pure((Cst.binder_char(tok, c)))
  end)
  end
  ...
  def parse_binder_bool() do
     ...
     Nova.Runtime.pure((Cst.binder_boolean(tok, true)))
   end)
      parse_binder_false =    Nova.Runtime.bind(tok_keyword("false"), fn tok ->
     Nova.Runtime.pure((Cst.binder_boolean(tok, false)))
   end)
  ...
  def parse_binder_array() do
    ...
    Nova.Runtime.pure((Cst.binder_array(d)))
  end) end)
  end
  ...
  def parse_binder_record() do
    ...
    Nova.Runtime.pure((Cst.binder_record(d)))
  end) end)
  end
  ...
  def parse_binder_parens() do
    ...
    Nova.Runtime.pure((Cst.binder_parens(w)))
  end) end)
  end
  ...
  def parse_module_imports() do
      ...
      go = Nova.Runtime.fix(fn go -> fn acc -> Nova.Runtime.alt((case parse_import() do
  {:left, err} -> {:left, err}
  {:right, imp} ->
  ...
  def parse_module_decls() do
      ...
      go = Nova.Runtime.fix(fn go -> fn acc -> Nova.Runtime.alt((case parse_declaration() do
  {:left, err} -> {:left, err}
  {:right, decl} ->
    Nova.Runtime.bind(optional(tok_layout_sep()), fn sep ->
      case sep do
  ...
  def parse_module_name() do
      ...
      extract_module_name = fn tok -> case tok do
        {:tok_upper_name, ({:just, prefix}), proper} -> {:just, (Cst.module_name((Nova.Runtime.append(Nova.Runtime.append(prefix, "."), proper))))}
        {:tok_upper_name, :nothing, proper} -> {:just, (Cst.module_name(proper))}
        _ -> :nothing
      end end
      case expect_map(extract_module_name) do
  {:left, err} -> {:left, err}
  {:right, {:tuple, tok, name}} ->
  ...
  def parse_export_value() do
    Nova.Runtime.fmap(Cst.export_value, tok_lower_name())
  end
  ...
  def parse_export_type() do
    ...
    Nova.Runtime.bind(optional(parse_data_members()), fn members ->
      Nova.Runtime.pure((Cst.export_type(name, members)))
    end)
  end)
  end
  ...
  def parse_export_module() do
    ...
    case parse_module_name() do
      {:left, err} -> {:left, err}
      {:right, name} ->
        Nova.Runtime.pure((Cst.export_module(kw, name)))
    end
  end)
  end
  ...
  def parse_data_all() do
    ...
    Nova.Runtime.bind(tok_double_dot(), fn dotdot ->
      Nova.Runtime.bind(tok_right_paren(), fn _ ->
        Nova.Runtime.pure((Cst.data_all(dotdot)))
      end)
    end)
  end)
  end
  ...
  def parse_data_enumerated() do
    ...
    Nova.Runtime.pure((Cst.data_enumerated(d)))
  end)
  end
  ...
  def parse_import_op() do
      Nova.Runtime.bind(tok_left_paren(), fn _ ->
    case parse_op_name() do
      {:left, err} -> {:left, err}
      {:right, op} ->
        Nova.Runtime.bind(tok_right_paren(), fn _ ->
          Nova.Runtime.pure((Cst.import_op(op)))
        end)
    end
  end)
  end
  ...
  def parse_import_value() do
    Nova.Runtime.fmap(Cst.import_value, tok_lower_name())
  end
  ...
  def parse_import_type() do
      Nova.Runtime.bind(tok_upper_name(), fn name ->
    Nova.Runtime.bind(optional(parse_data_members()), fn members ->
      Nova.Runtime.pure((Cst.import_type(name, members)))
    end)
  end)
  end
  ...
  def parse_import_class() do
    ...
    Nova.Runtime.bind(tok_upper_name(), fn name ->
      Nova.Runtime.pure((Cst.import_class(kw, name)))
    end)
  end)
  end
  ...
  Nova.Runtime.bind(separated(parse_data_ctor(), tok_pipe()), fn cs ->
    Nova.Runtime.pure(({:tuple, eq, cs}))
          ...
          Nova.Runtime.pure((Cst.decl_data(%{keyword: kw, name: name, vars: vars}, ctors)))
        end)
      end)
    end)
  end)
  ...
  def parse_type_var_name() do
    Nova.Runtime.fmap(Cst.type_var_name, tok_lower_name())
  end
      ...
      Nova.Runtime.bind(many(parse_type_var_binding()), fn vars ->
        Nova.Runtime.bind(tok_equals(), fn eq ->
          case parse_type() do
            {:left, err} -> {:left, err}
            {:right, ty} ->
              Nova.Runtime.pure((Cst.decl_type(%{keyword: kw, name: name, vars: vars}, eq, ty)))
          end
        end)
      end)
    end)
      ...
      Nova.Runtime.bind(many(parse_type_var_binding()), fn vars ->
        Nova.Runtime.bind(tok_equals(), fn eq ->
          Nova.Runtime.bind(tok_upper_name(), fn ctor_name ->
            case parse_type_atom() do
              {:left, err} -> {:left, err}
              {:right, wrapped_ty} ->
                Nova.Runtime.pure((Cst.decl_newtype(%{keyword: kw, name: name, vars: vars}, eq, ctor_name, wrapped_ty)))
            end
          end)
        end)
      end)
  ...
  Nova.Runtime.bind(layout_block(parse_class_member()), fn ms ->
    Nova.Runtime.pure(({:tuple, where_kw, ms}))
          ...
          Nova.Runtime.pure((Cst.decl_class(%{keyword: kw, super_: :nothing, name: name, vars: vars, fundeps: :nothing}, methods)))
        end)
      end)
    end)
  end)
  ...
  Nova.Runtime.bind(layout_block(parse_instance_binding()), fn bindings ->
    Nova.Runtime.pure(({:tuple, where_kw, bindings}))
            ...
            Nova.Runtime.pure((Cst.decl_instance_chain(%{head: inst, tail: []})))
          end)
        end)
      end)
    end)
  ...
  def parse_instance_sig() do
    ...
    Nova.Runtime.bind(tok_double_colon(), fn dc ->
      case parse_type() do
        {:left, err} -> {:left, err}
        {:right, ty} ->
          Nova.Runtime.pure((Cst.instance_binding_signature(%{label: name, separator: dc, value: ty})))
      end
    end)
  end)
  end
  ...
  def parse_instance_name() do
    ...
    Nova.Runtime.bind(many(parse_binder()), fn binders ->
      case parse_guarded() do
        {:left, err} -> {:left, err}
        {:right, guarded} ->
          Nova.Runtime.pure((Cst.instance_binding_name(%{name: name, binders: binders, guarded: guarded})))
      end
    end)
  end)
  end
    ...
    Nova.Runtime.pure(({:tuple, n, dc}))
          ...
          Nova.Runtime.bind(tok_qualified_upper_name(), fn class_name ->
            Nova.Runtime.bind(many(parse_type_atom()), fn types ->
              ...
              Nova.Runtime.pure((Cst.decl_derive(derive_kw, newtype_kw, instance_head)))
            end)
          end)
        end)
      end)
  ...
  def parse_decl_foreign() do
    ...
    Nova.Runtime.bind(tok_keyword("import"), fn import_ ->
      case parse_foreign_value() do
        {:left, err} -> {:left, err}
        {:right, foreign_prime} ->
          Nova.Runtime.pure((Cst.decl_foreign(foreign_, import_, foreign_prime)))
      end
    end)
  end)
  end
  ...
  def parse_foreign_value() do
    ...
    Nova.Runtime.bind(tok_double_colon(), fn dc ->
      case parse_type() do
        {:left, err} -> {:left, err}
        {:right, ty} ->
          Nova.Runtime.pure((Cst.foreign_value(%{label: name, separator: dc, value: ty})))
      end
    end)
  end)
  end
  ...
  def parse_decl_fixity() do
      ...
      case parse_fixity_keyword() do
  {:left, err} -> {:left, err}
  {:right, {:tuple, kw, fixity}} ->
    Nova.Runtime.bind(tok_int(), fn {:tuple, prec_tok, prec} ->
      case parse_fixity_op() do
        {:left, err} -> {:left, err}
        {:right, op} ->
          Nova.Runtime.pure((Cst.decl_fixity(%{keyword: {:tuple, kw, fixity}, prec: {:tuple, prec_tok, (int_value_to_int.(prec))}, operator: op})))
      end
    end)
end
  end
  ...
  def parse_fixity_keyword() do
     ...
     Nova.Runtime.pure(({:tuple, tok, Cst.infix}))
   end)
      parse_infixl =    Nova.Runtime.bind(tok_keyword("infixl"), fn tok ->
     Nova.Runtime.pure(({:tuple, tok, Cst.infixl}))
   end)
      parse_infixr =    Nova.Runtime.bind(tok_keyword("infixr"), fn tok ->
     Nova.Runtime.pure(({:tuple, tok, Cst.infixr}))
   end)
  ...
  Nova.Runtime.bind(tok_keyword("as"), fn as_kw ->
    case parse_op_name() do
      {:left, err} -> {:left, err}
      {:right, op} ->
        Nova.Runtime.pure((Cst.fixity_value((to_either_name.(name)), as_kw, op)))
    end
  end)
end)
  end
  ...
  def parse_decl_signature() do
    ...
    Nova.Runtime.bind(tok_double_colon(), fn dc ->
      case parse_type() do
        {:left, err} -> {:left, err}
        {:right, ty} ->
          Nova.Runtime.pure((Cst.decl_signature(%{label: name, separator: dc, value: ty})))
      end
    end)
  end)
  end
  ...
  def parse_decl_value() do
    ...
    Nova.Runtime.bind(many(parse_binder()), fn binders ->
      case parse_guarded() do
        {:left, err} -> {:left, err}
        {:right, guarded} ->
          Nova.Runtime.pure((Cst.decl_value(%{name: name, binders: binders, guarded: guarded})))
      end
    end)
  end)
  end
