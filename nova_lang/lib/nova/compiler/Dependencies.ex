...
defmodule Nova.Compiler.Dependencies do
  ...
  def get_dependencies(decl) do
    case decl do
      {:decl_function, f} -> 
          ...
          guard_deps = Data.Foldable.foldl((fn acc -> fn g -> Nova.Set.union(acc, (get_guarded_expr_deps(g))) end end), Nova.Set.empty, f.guards)
          param_names = Data.Foldable.foldl((fn acc -> fn p -> Nova.Set.union(acc, (get_bound_names(p))) end end), Nova.Set.empty, f.parameters)
          Nova.Set.difference((Nova.Set.union(body_deps, guard_deps)), param_names)
      {:decl_data_type, d} -> Data.Foldable.foldl((fn acc -> fn c -> Nova.Set.union(acc, (get_constructor_deps(c))) end end), Nova.Set.empty, d.constructors)
      {:decl_type_alias, a} -> get_type_expr_deps(a.ty)
      {:decl_type_class, c} -> Data.Foldable.foldl((fn acc -> fn m -> Nova.Set.union(acc, (get_type_expr_deps(m.ty))) end end), Nova.Set.empty, c.methods)
      {:decl_type_class_instance, i} -> 
          ty_deps = get_type_expr_deps(i.ty)
          method_deps = Data.Foldable.foldl((fn acc -> fn m -> Nova.Set.union(acc, (get_expr_deps(m.body))) end end), Nova.Set.empty, i.methods)
  ...
  def get_constructor_deps(c) do
    Data.Foldable.foldl((fn acc -> fn f -> Nova.Set.union(acc, (get_type_expr_deps(f.ty))) end end), Nova.Set.empty, c.fields)
  end
  ...
  def get_type_expr_deps(ty) do
    case ty do
      ...
      {:ty_expr_record, fields, _} -> Data.Foldable.foldl((fn acc -> fn ({:tuple, _, t}) -> Nova.Set.union(acc, (get_type_expr_deps(t))) end end), Nova.Set.empty, fields)
      {:ty_expr_for_all, _, t} -> get_type_expr_deps(t)
      {:ty_expr_constrained, cs, t} -> 
          constraint_deps = Data.Foldable.foldl((fn acc -> fn c -> Nova.Set.union(acc, (Nova.Set.singleton(c.class_name))) end end), Nova.Set.empty, cs)
          ty_deps = Data.Foldable.foldl((fn acc -> fn c -> Nova.Set.union(acc, (Data.Foldable.foldl((fn acc2 -> fn ct -> Nova.Set.union(acc2, (get_type_expr_deps(ct))) end end), Nova.Set.empty, c.types))) end end), Nova.Set.empty, cs)
          Nova.Set.union(constraint_deps, (Nova.Set.union(ty_deps, (get_type_expr_deps(t)))))
      {:ty_expr_parens, t} -> get_type_expr_deps(t)
      {:ty_expr_tuple, ts} -> Data.Foldable.foldl((fn acc -> fn t -> Nova.Set.union(acc, (get_type_expr_deps(t))) end end), Nova.Set.empty, ts)
    end
  end
  ...
  def get_expr_deps(expr) do
    case expr do
      ...
      {:expr_lambda, pats, body} -> 
          bound_names = Data.Foldable.foldl((fn acc -> fn p -> Nova.Set.union(acc, (get_bound_names(p))) end end), Nova.Set.empty, pats)
          Nova.Set.difference((get_expr_deps(body)), bound_names)
      {:expr_let, binds, body} -> 
          bind_deps = get_let_binds_deps(binds)
          bound_names = Data.Foldable.foldl((fn acc -> fn b -> Nova.Set.union(acc, (get_bound_names(b.pattern))) end end), Nova.Set.empty, binds)
      ...
      {:expr_case, e, clauses} -> Nova.Set.union((get_expr_deps(e)), (Data.Foldable.foldl((fn acc -> fn c -> Nova.Set.union(acc, (get_case_clause_deps(c))) end end), Nova.Set.empty, clauses)))
      ...
      {:expr_list, es} -> Data.Foldable.foldl((fn acc -> fn e -> Nova.Set.union(acc, (get_expr_deps(e))) end end), Nova.Set.empty, es)
      {:expr_tuple, es} -> Data.Foldable.foldl((fn acc -> fn e -> Nova.Set.union(acc, (get_expr_deps(e))) end end), Nova.Set.empty, es)
      {:expr_record, fields} -> Data.Foldable.foldl((fn acc -> fn ({:tuple, _, e}) -> Nova.Set.union(acc, (get_expr_deps(e))) end end), Nova.Set.empty, fields)
      {:expr_record_access, e, _} -> get_expr_deps(e)
      {:expr_record_update, e, fields} -> Nova.Set.union((get_expr_deps(e)), (Data.Foldable.foldl((fn acc -> fn ({:tuple, _, v}) -> Nova.Set.union(acc, (get_expr_deps(v))) end end), Nova.Set.empty, fields)))
  ...
  def get_let_binds_deps(binds) do
    Data.Foldable.foldl((fn acc -> fn b -> Nova.Set.union(acc, (get_expr_deps(b.value))) end end), Nova.Set.empty, binds)
  end
  ...
  def get_guarded_expr_deps(g) do
      ...
      guard_deps = Data.Foldable.foldl((fn acc -> fn gc -> Nova.Set.union(acc, (get_guard_clause_deps(gc))) end end), Nova.Set.empty, g.guards)
  ...
  def get_bound_names(pat) do
    case pat do
      ...
      {:pat_con, _, pats} -> Data.Foldable.foldl((fn acc -> fn p -> Nova.Set.union(acc, (get_bound_names(p))) end end), Nova.Set.empty, pats)
      {:pat_record, fields} -> Data.Foldable.foldl((fn acc -> fn ({:tuple, _, p}) -> Nova.Set.union(acc, (get_bound_names(p))) end end), Nova.Set.empty, fields)
      {:pat_list, pats} -> Data.Foldable.foldl((fn acc -> fn p -> Nova.Set.union(acc, (get_bound_names(p))) end end), Nova.Set.empty, pats)
  ...
  def build_dependency_graph(decls, resolve_name) do
      ...
      forward = Data.Foldable.foldl((fn acc -> fn id -> case Nova.Map.lookup(id, decls) do
        :nothing -> acc
        {:just, md} -> 
  ...
  def build_reverse_edges(forward) do
      ...
      Data.Foldable.foldl((fn acc -> fn ({:tuple, from_id, deps}) -> 
  deps_array = Nova.Set.to_unfoldable(deps)
  Data.Foldable.foldl((fn acc2 -> fn to_id -> 
  ...
  def add_to_graph(decl_id, deps, graph) do
      ...
      deps_array = Nova.Set.to_unfoldable(deps)
      reverse = Data.Foldable.foldl((fn acc -> fn dep_id -> 
  ...
  def remove_from_graph(decl_id, graph) do
      ...
      deps_array = Nova.Set.to_unfoldable(deps)
      reverse = Data.Foldable.foldl((fn acc -> fn dep_id -> case Nova.Map.lookup(dep_id, acc) do
        :nothing -> acc
  ...
  def topological_sort(graph, ids) do
      ...
      initial_in_degree = Data.Foldable.foldl((fn acc -> fn id -> 
      ...
      go = Nova.Runtime.fix3(fn go -> fn in_degree -> fn result -> fn queue -> case Nova.Array.uncons(queue) do
        ...
        {:just, %{head: id, tail: rest_queue}} -> 
            ...
            new_in_degree = Data.Foldable.foldl((fn acc -> fn dep -> case Nova.Map.lookup(dep, acc) do
              :nothing -> acc
