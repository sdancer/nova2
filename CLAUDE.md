...
- `scripts/regenerate-purs.js` - Node wrapper that invokes `src/Nova/Compiler/Regenerate.purs` to recompile source to Elixir
...
node scripts/regenerate-purs.js    # Recompile source to Elixir (regenerate nova_lang/)
...
- **Regenerate.purs** - Self-compilation orchestrator (reads source files, compiles, writes output)
...
3. Regenerate Elixir: `node scripts/regenerate-purs.js`
4. Test compiled output: `cd nova_lang && mix test`

**Important:** After every round of changes, verify the compiler can still compile its own source code by running `node scripts/regenerate-purs.js`.
...
**Validate Theories with Tests Before Changing the Compiler:** Before making changes to fix type errors or other compiler issues, validate your theory with a minimal test case. If valid PureScript code compiles with the original PureScript compiler (`npx spago build`), our self-hosted compiler should handle it too (except for explicitly unsupported features like type classes). Create a small test file that isolates the problematic pattern, verify it compiles with spago, then use that to guide the fix.
