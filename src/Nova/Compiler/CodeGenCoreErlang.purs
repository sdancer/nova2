...
-- | Sequence an array of Eithers - returns first Left or all Rights
sequenceEithers :: forall a b. Array (Either a b) -> Either a (Array b)
sequenceEithers arr = go arr []
  where
    go :: Array (Either a b) -> Array b -> Either a (Array b)
    go remaining acc = case Array.uncons remaining of
      Nothing -> Right acc
      Just { head: Left err, tail: _ } -> Left err
      Just { head: Right val, tail: rest } -> go rest (acc <> [val])
...
-- | Generate module
genModule :: Module -> Either String String
genModule m =
  let modName = translateModuleName m.name
      ...
      -- Generate foreign import function definitions (may fail)
      foreignResults = map (genForeignImport m.name) allForeignImports
  ...
  in case sequenceEithers foreignResults of
    Left err -> Left err
    Right foreignDefsArr ->
      let foreignDefs = String.joinWith "\n\n" foreignDefsArr
      in Right ("module " <> atom modName <> " [" <> exports <> "]\n" <>
         "  attributes []\n" <>
...
-- Uses inlineImpl if provided, otherwise returns compile-time error
genForeignImport :: String -> ForeignImport -> Either String String
genForeignImport modName fi =
  let funcName = fi.functionName
      arity = countTypeArity fi.typeSignature
      params = if arity <= 0 then [] else map (\i -> "V" <> show i) (Array.range 0 (arity - 1))
      paramStr = String.joinWith ", " params
  in case fi.inlineImpl of
    Just impl ->
      let body = substituteParams impl params
      in Right ("'" <> funcName <> "'/" <> show arity <> " = fun (" <> paramStr <> ") -> " <> body)
    Nothing ->
      Left ("FFI function '" <> funcName <> "' in module '" <> modName <> "' has no inline implementation")
