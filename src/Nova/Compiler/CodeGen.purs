...
defaultCtx :: GenCtx
defaultCtx = emptyCtx { knownArities = defaultKnownArities }
...
defaultKnownArities = Map.fromFoldable
  ...
  , Tuple "fold" 1, Tuple "foldM" 3, Tuple "traverse_" 2, Tuple "for_" 2
  -- Partial.Unsafe functions
  , Tuple "unsafePartial" 1
  -- Test.Assert functions
  , Tuple "assert" 1, Tuple "assert'" 2
  -- Effect.Uncurried functions
  , Tuple "runEffectFn1" 2, Tuple "runEffectFn2" 3, Tuple "runEffectFn3" 4
  , Tuple "runEffectFn4" 5, Tuple "runEffectFn5" 6, Tuple "runEffectFn6" 7
  , Tuple "runEffectFn7" 8, Tuple "runEffectFn8" 9, Tuple "runEffectFn9" 10
  , Tuple "runEffectFn10" 11
  ...
  then snakeCase name
    ...
    _ ->
      ...
      then case lookupArity name ctx of
             ...
             Nothing ->
               -- Try type environment as fallback
               case lookupTypeArity name ctx of
                 Just typeArity ->
                   if typeArity == 0 then snakeCase name <> "()"
      ...
      -- Check if this is an imported function and resolve to the target module
        ...
        Just srcMod ->
          -- Use source module directly with arity from type env or knownArities
          case lookupTypeArity name ctx of
            Just funcArity ->
              "(&" <> srcMod <> "." <> snakeCase name <> "/" <> show funcArity <> ")"
            Nothing ->
              -- COMPILE ERROR: Missing type information for function arity
              unsafeCrashWith ("CodeGen error: Missing type arity for imported function '" <> name <> "' from module " <> srcMod <> "'")
        Nothing ->
  ...
  where
    ...
    genVarApp c i n exprs argsS =
      ...
      then case lookupArity n c of
             ...
             Nothing ->
               -- COMPILE ERROR: Missing arity for module function
               -- Try type environment lookup as fallback
               case lookupTypeArity n c of
                 Just typeArity ->
                   ...
                   in if numArgs == typeArity
                      ...
                      then genPartialApp (snakeCase n) argsS numArgs typeArity
                      else
      ...
      then
        ...
        in case lookupTypeArity n c of
             ...
             Just funcArity ->
               ...
               then genPartialApp qualifiedFunc argsS numArgs funcArity
               else if funcArity == 0
               then qualifiedFunc <> "(" <> argsS <> ")"
               else
                 -- More args than expected - function returns a function
...
genLetBind = genLetBindCtx defaultCtx
...
genCaseClause = genCaseClauseCtx defaultCtx
...
genDoStmts = genDoStmtsCtx defaultCtx
